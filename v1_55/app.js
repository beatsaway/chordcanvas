...
let loopMode = loopModeSelect?.value || 'all';
...
async function startPreview({ fromLoop = false, startAt = null } = {}) {
    ...
    const startAtSegmentIndex = Math.max(0, Math.min((startAt?.segmentIndex ?? 0), segments.length - 1));
    const startAtChordIndex = Math.max(0, startAt?.chordIndex ?? 0);
    let didClearHighlights = false;
    ...
    segments.forEach((segment, segmentIndex) => {
        ...
        segment.chordSequence.forEach((chord, index) => {
            ...
            const chordStart = startTime + (relativeIndex * barSeconds);
            ...
            schedulePreviewTimer(() => {
                if (isSequencePreviewing) {
                    if (!didClearHighlights) {
                        clearAllHighlights();
                        didClearHighlights = true;
                    }
        ...
        startTime += Math.max(0, segment.chordSequence.length - chordStartIndex) * barSeconds;
    });
...
function updatePreviewToggleLabel() {
    ...
    if (label) {
        label.textContent = isSequencePreviewing ? 'Stop' : 'Play';
    } else if (previewToggle) {
        previewToggle.textContent = isSequencePreviewing ? 'Stop' : 'Play';
    }
}
...
if (loopModeSelect) {
    loopModeSelect.addEventListener('change', (event) => {
        loopMode = event.target.value;
    });
}
