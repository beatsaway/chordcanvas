...
(function() {
    ...
    function createSettingsUI() {
        ...
        panel.innerHTML = `
            ...
            <div id="settings-panel-keyboard" class="settings-panel active" role="tabpanel">
                ...
                <div class="keyboard-visual-setting-item">
                    <label>
                        <input type="checkbox" id="enable-key-movement">
                        <div>
                ...
                <div class="keyboard-visual-setting-item">
                    <label>
                        <input type="checkbox" id="enable-midi-input">
            ...
            <div id="settings-panel-sound" class="settings-panel" role="tabpanel">
                ...
                <div class="keyboard-visual-setting-item">
                    <label>
                        <span class="setting-label-inline">Sound effect</span>
                        <select class="js-sound-mode-select" title="Normal, tape/ramp (Nostalgia), or pitch tremolo">
                ...
                <div class="keyboard-visual-setting-item">
                    <label>
                        ...
                        <input type="range" id="settings-master-volume" min="0" max="2000" value="700" step="10">
                        <span class="settings-master-volume-value" id="settings-master-volume-value">700%</span>
                    </label>
                </div>
            </div>
    ...
    function setupEventListeners() {
        ...
        document.body.addEventListener('change', function (e) {
            if (!e.target || !e.target.classList || !e.target.classList.contains('js-sound-mode-select')) return;
            if (window.synth && window.synth.setSoundMode) {
                var val = e.target.value;
                window.synth.setSoundMode(val);
                document.querySelectorAll('.js-sound-mode-select').forEach(function (other) {
                    if (other !== e.target) other.value = val;
                });
            }
        });
        if (window.synth && (window.synth.getSoundMode || window.synth.getNostalgiaMode)) {
            var syncSoundMode = function () {
                var mode = (window.synth.getSoundMode && window.synth.getSoundMode()) || (window.synth.getNostalgiaMode && window.synth.getNostalgiaMode() ? 'nostalgia' : 'normal');
                document.querySelectorAll('.js-sound-mode-select').forEach(function (sel) { sel.value = mode; });
            };
            syncSoundMode();
        }
        ...
        // When user clicks disabled restricted settings, open plan modal
        var keyboardPanel = document.getElementById('settings-panel-keyboard');
        if (keyboardPanel) {
            keyboardPanel.addEventListener('click', function (e) {
                var restrictedIds = ['enable-key-highlight', 'enable-key-movement', 'enable-key-labels', 'enable-keypress-input', 'camera-enabled', 'camera-help-btn'];
                for (var i = 0; i < restrictedIds.length; i++) {
                    var el = document.getElementById(restrictedIds[i]);
                    if (!el || !el.disabled) continue;
                    var hit = (e.target === el || (el.contains && el.contains(e.target)));
                    if (!hit && el.parentElement && el.parentElement.tagName === 'LABEL') {
                        hit = (e.target === el.parentElement || el.parentElement.contains(e.target));
                    }
                    if (hit) {
                        e.preventDefault();
                        e.stopPropagation();
                        if (window.chordcanvasAuth && window.chordcanvasAuth.openPlanModal) {
                            window.chordcanvasAuth.openPlanModal();
                        }
                        return;
                    }
                }
        ...
        if (movementSettingsBtn) {
            movementSettingsBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                if (movementSettingsBtn.dataset.advancedRestricted === '1' && window.chordcanvasAuth && window.chordcanvasAuth.openPlanModal) {
                    window.chordcanvasAuth.openPlanModal();
                    return;
                }
        ...
        if (labelsCheckbox) {
            labelsCheckbox.addEventListener('change', (e) => {
                ...
                if (window.updateDivLabelsVisibility) {
                    window.updateDivLabelsVisibility();
                }
                updateUI();
            });
        }
        ...
        if (labelsSettingsBtn) {
            labelsSettingsBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                if (labelsSettingsBtn.dataset.advancedRestricted === '1' && window.chordcanvasAuth && window.chordcanvasAuth.openPlanModal) {
                    window.chordcanvasAuth.openPlanModal();
                    return;
                }
    ...
    /**
     * Apply plan restrictions: free and guests cannot change Key highlight, Key movement, Key labels,
     * Computer keyboard, Camera Control. pro, pro+, ultra can. MIDI input is always adjustable.
     * Only pro+ and ultra can open the ? advanced settings for Key movement and Key labels.
     * Exposed so it can be called when popup/tab is shown (auth may have loaded after init).
     */
    window.updateKeyboardPlanRestrictions = function () {
        applyPlanRestrictions();
    };

    function applyPlanRestrictions() {
        var auth = window.chordcanvasAuth;
        var rawPlan = (auth && auth.getPlan) ? auth.getPlan() : 'free';
        var plan = String(rawPlan || 'free').toLowerCase().replace(/-/g, '_').replace(/ /g, '');
        // free = restrict. pro, pro+, ultra = allow checkboxes. pro+, ultra = allow ? advanced.
        var isFree = (plan === 'free' || !plan);
        var canUseAdvancedSettings = (plan === 'pro+' || plan === 'proplus' || plan === 'pro_plus' || plan === 'ultra' || plan === 'aura');
        var restrictedIds = ['enable-key-highlight', 'enable-key-movement', 'enable-key-labels', 'enable-keypress-input', 'camera-enabled'];
        restrictedIds.forEach(function (id) {
            var el = document.getElementById(id);
            if (el) {
                el.disabled = isFree;
                el.title = isFree ? 'Sign in and upgrade to Pro to customize' : '';
            }
        });
        var restrictedBtnIds = ['camera-help-btn'];
        restrictedBtnIds.forEach(function (id) {
            var el = document.getElementById(id);
            if (el) {
                el.disabled = isFree;
                el.style.pointerEvents = isFree ? 'none' : '';
                el.style.opacity = isFree ? '0.5' : '';
                el.title = isFree ? 'Sign in and upgrade to Pro to customize' : (el.getAttribute('aria-label') || '');
            }
        });
        var advancedSettingsBtnIds = ['key-movement-settings-btn', 'key-labels-settings-btn'];
        advancedSettingsBtnIds.forEach(function (id) {
            var el = document.getElementById(id);
            if (el) {
                var disabled = !canUseAdvancedSettings;
                el.setAttribute('aria-disabled', disabled ? 'true' : 'false');
                el.style.opacity = disabled ? '0.5' : '';
                el.title = disabled ? 'pro+ or ultra plan required for advanced settings' : (el.getAttribute('aria-label') || '');
                el.dataset.advancedRestricted = disabled ? '1' : '0';
            }
        });
    }
    ...
    window.showKeyboardVisualSettings = function() {
        ...
        if (settingsModal) {
            if (settingsModal.id === 'app-popup') {
                ...
                if (window.synth && (window.synth.getSoundMode || window.synth.getNostalgiaMode)) {
                    var mode = (window.synth.getSoundMode && window.synth.getSoundMode()) || (window.synth.getNostalgiaMode && window.synth.getNostalgiaMode() ? 'nostalgia' : 'normal');
                    settingsModal.querySelectorAll('.js-sound-mode-select').forEach(function (sel) { sel.value = mode; });
                }
            }
            updateUI();
        }
