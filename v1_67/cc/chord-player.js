...
(function () {
    ...
    function getDefaultSoundState(forFirstRow) {
        ...
        if (typeof window !== 'undefined' && window.ccReadSoundStateFromGlobals) {
            ...
            // Default to no every-bar modulation so page load sounds purely piano (sister request)
    ...
    function readSoundStateFromGlobals() {
        ...
        return { presetSlots: slots, slotVolumes: vols, slotSemitones: semis, slotMuted: muted, everyBarPattern: everyBar, everyBarIntensity: everyBarInt, layerPlayStyle, layerPlayMode };
    }
    ...
    function getDefaultRowSettings() {
        return {
            bpm: CC_DEFAULT_BPM,
            bassDurationMultiplier: 30,
            trebleDurationMultiplier: 30,
            skipPattern: [],
            bassSkipPattern: [],
            trebleSkipPattern: [],
            patternMode: 'normal',
            bassPatternMode: 'normal',
            velocity: CC_DEFAULT_VELOCITY,
            octaveShift: 0,
            voicing: 'full',
            trebleOctaves: 1,
            bassOctaves: 1,
    ...
    function readRowSettingsFromUi() {
        const bpmInput = document.getElementById('cc-bpm');
        ...
        const velocityEl = document.getElementById('cc-velocity');
        ...
        const velocityRaw = velocityEl ? parseInt(velocityEl.value, 10) : CC_DEFAULT_VELOCITY;
        const velocity = Number.isFinite(velocityRaw) ? Math.max(1, Math.min(127, velocityRaw)) : CC_DEFAULT_VELOCITY;
        ...
        const delayMod = delayModEl ? String(delayModEl.value || 'drunk') : 'drunk';
        ...
        return {
            ...
            delayMod: CC_DELAY_MOD_OPTIONS.includes(delayMod) ? delayMod : 'drunk',
            delayIntensity: 1,
    ...
    function applyRowSettingsToUi(s) {
        ...
        setIf('cc-bpm', clampBpm(s.bpm ?? CC_DEFAULT_BPM));
        ...
        setIf('cc-velocity', s.velocity ?? CC_DEFAULT_VELOCITY);
        ...
        setIf('cc-delay-mod', s.delayMod ?? 'drunk');
    ...
    const CC_EMO_EXTENSIONS = [
        ['', 'add2', 'add4', 'add9', 'maj7', 'maj9', 'maj11', 'maj13'], // I
        ['', '7', '9', '11', '13'],                                     // ii (minor)
        ['', '7', '9', '11', '13'],                                     // iii (minor)
    ...
    function addChordRowAfter(index, persistCb) {
        ...
        row.innerHTML = '<div class="cc-chord-row-drag" draggable="true" title="Tap: Add/Delete · Drag: reorder" aria-label="Tap for options or drag to reorder">⋮</div><div class="cc-chord-input-wrap"><textarea class="cc-chord-input" data-index="' + (index + 1) + '" maxlength="120" placeholder="Enter chords separated by commas (e.g. C, Am, F, G7)"></textarea></div>';
        ...
        const soundCopy = (s) => s ? { presetSlots: (s.presetSlots || []).slice(), slotVolumes: (s.slotVolumes || []).slice(), slotSemitones: (s.slotSemitones || []).slice(), slotMuted: (s.slotMuted || []).slice(), everyBarPattern: (s.everyBarPattern || []).slice(), everyBarIntensity: (s.everyBarIntensity || []).slice(), layerPlayStyle: s.layerPlayStyle != null ? s.layerPlayStyle : 'none', layerPlayMode: (s.layerPlayMode === 'split' || s.layerPlayMode === 'scatter') ? s.layerPlayMode : 'leftRight' } : getDefaultSoundState();
        rowSettings.splice(index + 1, 0, Object.assign({}, getDefaultRowSettings(), rowSettings[index] || {}));
        rowSoundState.splice(index + 1, 0, soundCopy(rowSoundState[index]));
    ...
    function buildCompiledState(state) {
        ...
        return {
            ...
            velocity: state.velocity != null ? Math.max(1, Math.min(127, state.velocity)) : CC_DEFAULT_VELOCITY,
    ...
    function readStateFromUi() {
        ...
        const soundCopy = (s) => !s ? getDefaultSoundState() : { presetSlots: (s.presetSlots || []).slice(), slotVolumes: (s.slotVolumes || []).slice(), slotSemitones: (s.slotSemitones || []).slice(), slotMuted: (s.slotMuted || []).slice(), everyBarPattern: (s.everyBarPattern || []).slice(), everyBarIntensity: (s.everyBarIntensity || []).slice(), layerPlayStyle: s.layerPlayStyle != null ? s.layerPlayStyle : 'none', layerPlayMode: (s.layerPlayMode === 'split' || s.layerPlayMode === 'scatter') ? s.layerPlayMode : 'leftRight' };
        return {
            chordsText,
            chordsTextArray,
            chordsLastFocusedIndex: lastFocusedChordIndex,
            settingsPerRow: rowSettings.slice(),
            soundPerRow: rowSoundState.map(soundCopy),
            bpm: s.bpm,
            ...
            velocity: s.velocity ?? CC_DEFAULT_VELOCITY,
    ...
    function ensureRowSoundStateLength(n) {
        const copy = (s) => s ? { presetSlots: (s.presetSlots || []).slice(), slotVolumes: (s.slotVolumes || []).slice(), slotSemitones: (s.slotSemitones || []).slice(), slotMuted: (s.slotMuted || []).slice(), everyBarPattern: (s.everyBarPattern || []).slice(), everyBarIntensity: (s.everyBarIntensity || []).slice(), layerPlayStyle: s.layerPlayStyle != null ? s.layerPlayStyle : 'none', layerPlayMode: (s.layerPlayMode === 'split' || s.layerPlayMode === 'scatter') ? s.layerPlayMode : 'leftRight' } : getDefaultSoundState();
        while (rowSoundState.length < n) rowSoundState.push(rowSoundState.length === 0 ? getDefaultSoundState(true) : copy(rowSoundState[rowSoundState.length - 1]));
        ...
        if (state.chordsTextArray && Array.isArray(state.chordsTextArray) && state.chordsTextArray.length > 0) {
            ...
            if (container) {
                ...
                texts.forEach((text, i) => {
                    ...
                    row.innerHTML = '<div class="cc-chord-row-drag" draggable="true" title="Tap: Add/Delete · Drag: reorder" aria-label="Tap for options or drag to reorder">⋮</div><div class="cc-chord-input-wrap"><textarea class="cc-chord-input" data-index="' + i + '" maxlength="120" placeholder="Enter chords separated by commas (e.g. C, Am, F, G7)"></textarea></div>';
                    const ta = row.querySelector('.cc-chord-input');
                ...
                if (state.soundPerRow && Array.isArray(state.soundPerRow) && state.soundPerRow.length) {
                    rowSoundState = state.soundPerRow.slice(0, CC_CHORD_MAX_ROWS).map((s) => s ? { presetSlots: (s.presetSlots || []).slice(), slotVolumes: (s.slotVolumes || []).slice(), slotSemitones: (s.slotSemitones || []).slice(), slotMuted: (s.slotMuted || []).slice(), everyBarPattern: (s.everyBarPattern || []).slice(), everyBarIntensity: (s.everyBarIntensity || []).slice(), layerPlayStyle: s.layerPlayStyle != null ? s.layerPlayStyle : 'none', layerPlayMode: (s.layerPlayMode === 'split' || s.layerPlayMode === 'scatter') ? s.layerPlayMode : 'leftRight' } : getDefaultSoundState());
                } else {
                    rowSoundState = [];
                }
            ...
            if (!rowSettings.length) rowSettings = [state.settingsPerRow && state.settingsPerRow[0] ? state.settingsPerRow[0] : {
                ...
                skipPattern: state.skipPattern, patternMode: state.patternMode, bassPatternMode: state.bassPatternMode, velocity: state.velocity ?? CC_DEFAULT_VELOCITY, octaveShift: state.octaveShift,
            ...
            if (!rowSoundState.length) rowSoundState = state.soundPerRow && state.soundPerRow.length ? state.soundPerRow.slice().map((s) => s ? { presetSlots: (s.presetSlots || []).slice(), slotVolumes: (s.slotVolumes || []).slice(), slotSemitones: (s.slotSemitones || []).slice(), slotMuted: (s.slotMuted || []).slice(), everyBarPattern: (s.everyBarPattern || []).slice(), everyBarIntensity: (s.everyBarIntensity || []).slice(), layerPlayStyle: s.layerPlayStyle != null ? s.layerPlayStyle : 'none', layerPlayMode: (s.layerPlayMode === 'split' || s.layerPlayMode === 'scatter') ? s.layerPlayMode : 'leftRight' } : getDefaultSoundState()) : [getDefaultSoundState(true)];
        }
        ensureRowSoundStateLength(getChordRows().length);
    ...
    function init() {
        ...
        if (chordContainer) {
            ...
            function dragRowMenuOutsideClick(e) {
                ...
                if (chordContainer && chordContainer.contains(e.target)) return;
                closeDragRowMenu();
            }
            ...
            chordContainer.addEventListener('input', () => saveState(readStateFromUi()));
        ...
        const bpmEl = document.getElementById('cc-bpm');
        ...
        const rebuildRhythmOptions = (clampInput) => {
            ...
            if (bassSel) bassSel.value = prevBass && CC_DURATION_MULTIPLIERS.includes(parseInt(prevBass, 10)) ? prevBass : String(30);
        ...
        if (skip1) skip1.value = '0';
        if (skip2) skip2.value = '0';
        ...
        if (bassEveryBarEl) bassEveryBarEl.value = 'uphill';
        if (trebleEveryBarEl) trebleEveryBarEl.value = 'uphill';
        ...
        [
            'cc-bpm', 'cc-bass-rhythm', 'cc-treble-rhythm',
            'cc-bass-skip1', 'cc-bass-skip2', 'cc-bass-skip3',
            'cc-skip1', 'cc-skip2', 'cc-skip3',
            'cc-pattern-mode', 'cc-bass-pattern-mode', 'cc-velocity', 'cc-octave-shift', 'cc-voicing', 'cc-double-treble', 'cc-double-bass'
        ...
        if (bpmEl) {
            bpmEl.addEventListener('input', () => rebuildRhythmOptions(false));
            bpmEl.addEventListener('change', () => rebuildRhythmOptions(true));
            bpmEl.addEventListener('blur', () => {
                const clamped = clampBpm(bpmEl.value);
                if (Number(bpmEl.value) !== clamped) bpmEl.value = String(clamped);
        ...
        playBtn.addEventListener('click', () => {
            ...
            saveState(state);
            // Mobile: ensure AudioContext is running (resume if suspended) then load presets, then start with delay. Order matches ChordCanvas ensureReady + ensurePresetLoaded.
            ensureSynthAudioReady().then(function () {
                ...
                // Preload next row(s) in background so chained rows start without glitch (n-ahead idea from ChordCanvas).
        ...
        window.ccExport = function (kind) {
            ...
            if (kind === 'midi') {
                try {
                    ...
                    a.href = url;
                    a.download = `primidi-chords-${Date.now()}.mid`;
            ...
            if (kind === 'wav') {
                (async () => {
                    try {
                        ...
                        const a = document.createElement('a');
                        a.href = url;
                        a.download = `primidi-chords-${Date.now()}.wav`;
                        document.body.appendChild(a);
                        a.click();
                        document.body.removeChild(a);
