...
// Uses shared masterAudioContext from index.html
let masterGain;
// bassMasterCompressor for bass synth chain (separate from index.html's masterCompressor)
let bassMasterCompressor;
...
function initMasterChain() {
    const audioContext = window.masterAudioContext;
    ...
    if (!bassMasterCompressor) {
        bassMasterCompressor = audioContext.createDynamicsCompressor();
        bassMasterCompressor.threshold.value = -20; // Start compressing at -20dB
        bassMasterCompressor.knee.value = 30; // Soft knee
        bassMasterCompressor.ratio.value = 3; // 3:1 ratio
        bassMasterCompressor.attack.value = 0.003; // Fast attack (3ms)
        bassMasterCompressor.release.value = 0.1; // Quick release (100ms)
    }
    ...
    sidechainGainGrowl.connect(bassMasterCompressor);
    bassMasterCompressor.connect(masterBrightness);
    ...
    try {
        masterBoost.disconnect();
    } catch (e) {
        // May not be connected, ignore
    }
    
    if (window.masterCompressor) {
        masterBoost.connect(window.masterCompressor);
    } else {
...
function initAudioContext() {
    ...
    if (!masterGain) {
        ...
        masterGain.gain.value = 0.2; // Default 20% (lowered from 40%)
...
function ensureMasterGain() {
    ...
    if (!masterGain && audioContext) {
        masterGain = audioContext.createGain();
        const defaultVolume = volumeSlider ? parseFloat(volumeSlider.value) / 100 : 0.2; // Lowered from 0.4
...
// Shared helper functions for all bass types
// These functions handle common operations like lowpass filtering and volume control

/**
 * Creates a shared updateLowpass function that works with any filter node
 * @param {Function} getFilterNode - Function that returns the filter node to control (or filter2 for aggressive type)
 * @returns {Function} updateLowpass function
 */
function createUpdateLowpass(getFilterNode) {
    return (lowpassValue) => {
        // lowpassValue is 0-1, map to frequency range (50Hz to 2000Hz)
        const audioContext = window.masterAudioContext;
        if (!audioContext) return;
        ...
        const minFreq = 50;
        const maxFreq = 2000;
        const frequency = minFreq + (maxFreq - minFreq) * lowpassValue;
        filterNode.frequency.cancelScheduledValues(now);
        filterNode.frequency.setValueAtTime(filterNode.frequency.value, now);
        filterNode.frequency.linearRampToValueAtTime(frequency, now + 0.01);
    };
}

/**
 * Creates a shared updateVolume function that works with any gain node
 * @param {Function} getGainNode - Function that returns the gain node to control
 * @param {number} baseSustain - Base sustain level (0-1) for this bass type
 * @returns {Function} updateVolume function
 */
function createUpdateVolume(getGainNode, baseSustain) {
    return (volumeMultiplier) => {
        // volumeMultiplier is 0-1, adjust gain node sustain level
        const audioContext = window.masterAudioContext;
        if (!audioContext) return;
        const gainNode = getGainNode();
        if (!gainNode) return;
        const now = audioContext.currentTime;
        const targetGain = baseSustain * volumeMultiplier;
        gainNode.gain.cancelScheduledValues(now);
        gainNode.gain.setValueAtTime(gainNode.gain.value, now);
        gainNode.gain.linearRampToValueAtTime(targetGain, now + 0.01);
        gainNode.gain.setValueAtTime(targetGain, now + 0.01 + 0.0001);
    };
}
...
// 
// FUNDAMENTAL FREQUENCY/PITCH PARAMETER:
// - Each sound creator accepts 'rootFrequency' parameter (default: 55Hz)
// - This is the fundamental frequency that determines the pitch of the bass
// - Set via: carrier.frequency.value = rootFrequency (in the start() function)
// - Modulators are scaled relative to rootFrequency (e.g., rootFrequency * 2 for 2:1 FM ratio)
// - When called from index.html, it passes: lowestNoteFreq / 4 (two octaves lower than palette's lowest note)
const soundCreators = {
    classic: (rootFrequency = 55) => {
        ...
        const start = () => {
            ...
            nodes = { carrier, modulator, lfo, gainNode, filter };
        };
        ...
        const updatePitch = (pitchBendMultiplier) => {
            const audioContext = window.masterAudioContext;
            if (!nodes.carrier || !nodes.modulator || !audioContext) return;
            const now = audioContext.currentTime;
        ...
        // Pass a getter function so nodes.filter is accessed when updateLowpass is called, not when it's created
        const updateLowpass = createUpdateLowpass(() => nodes.filter);
        const updateVolume = createUpdateVolume(() => gainNode, 0.4); // baseSustain = 0.4 for classic
        
        return { start, stop, updatePitch, updateLowpass, updateVolume, get lfo() { return lfo; }, get baseFrequency() { return rootFrequency; }, get filter() { return nodes.filter; }, get gainNode() { return gainNode; } };
    },
    ...
    wobble: (rootFrequency = 55) => {
        ...
        // Use shared helper functions for common operations
        const updateLowpass = createUpdateLowpass(() => nodes.filter);
        const updateVolume = createUpdateVolume(() => gainNode, 0.2); // baseSustain = 0.2 for wobble
    ...
    roar: (rootFrequency = 55) => {
        ...
        const start = () => {
            ...
            nodes = { carrier, modulator1, modulator2, lfo, gainNode, filter };
        };
        ...
        const updatePitch = (pitchBendMultiplier) => {
            const audioContext = window.masterAudioContext;
            if (!nodes.carrier || !nodes.modulator1 || !nodes.modulator2 || !audioContext) return;
            const now = audioContext.currentTime;
            const newCarrierFreq = rootFrequency * pitchBendMultiplier;
        ...
        // Use shared helper functions for common operations
        const updateLowpass = createUpdateLowpass(() => nodes.filter);
        const updateVolume = createUpdateVolume(() => gainNode, 0.45); // baseSustain = 0.45 for roar
    ...
    beast: (rootFrequency = 55) => {
        ...
        const start = () => {
            ...
            nodes = { carrier1, carrier2, modulator1, modulator2, lfo, gainNode, filter };
        };
        ...
        const updatePitch = (pitchBendMultiplier) => {
            const audioContext = window.masterAudioContext;
            if (!nodes.carrier1 || !nodes.carrier2 || !nodes.modulator1 || !nodes.modulator2 || !audioContext) return;
            const now = audioContext.currentTime;
        ...
        // Use shared helper functions for common operations
        const updateLowpass = createUpdateLowpass(() => nodes.filter);
        const updateVolume = createUpdateVolume(() => gainNode, 0.25); // baseSustain = 0.25 for beast
    ...
    sweep: (rootFrequency = 55) => {
        ...
        // Use shared helper functions for common operations
        const updateLowpass = createUpdateLowpass(() => nodes.filter);
        const updateVolume = createUpdateVolume(() => gainNode, 0.25); // baseSustain = 0.25 for sweep
    ...
    stutter: (rootFrequency = 55) => {
        ...
        // Use shared helper functions for common operations
        // Note: stutter uses filter, but updateLowpass needs to access nodes.filter dynamically
        const updateLowpass = createUpdateLowpass(() => nodes.filter);
        const updateVolume = createUpdateVolume(() => gainNode, 0.1); // baseSustain = 0.1 for stutter
        ...
        return { 
            ...
            get filter() { return nodes.filter; },
    ...
    deep: (rootFrequency = 55) => {
        ...
        // Use shared helper functions for common operations
        const updateLowpass = createUpdateLowpass(() => nodes.filter);
        const updateVolume = createUpdateVolume(() => gainNode, 0.45); // baseSustain = 0.45 for deep
    ...
    aggressive: (rootFrequency = 55) => {
        ...
        const start = () => {
            ...
            nodes = { carrier1, carrier2, carrier3, modulator, lfo, gainNode, filter1, filter2 };
        };
        ...
        const updatePitch = (pitchBendMultiplier) => {
            const audioContext = window.masterAudioContext;
            if (!nodes.carrier1 || !nodes.carrier2 || !nodes.carrier3 || !nodes.modulator || !audioContext) return;
            const now = audioContext.currentTime;
            const newCarrier1Freq = rootFrequency * pitchBendMultiplier;
        ...
        // Use shared helper functions for common operations
        // Note: aggressive uses filter2 (lowpass) instead of filter1 (bandpass)
        const updateLowpass = createUpdateLowpass(() => nodes.filter2);
        const updateVolume = createUpdateVolume(() => gainNode, 0.3); // baseSustain = 0.3 for aggressive
        
        return { start, stop, updatePitch, updateLowpass, updateVolume, get lfo() { return lfo; }, get baseFrequency() { return rootFrequency; }, get filter() { return nodes.filter2; }, get gainNode() { return gainNode; } };
    }
};
