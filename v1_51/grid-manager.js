...
class GridManager {
    ...
    createGrid(gridId, synthType) {
        ...
        const headers = this.calculateHeaders(currentBPM);
    ...
    // Calculate headers based on current BPM
    calculateHeaders(bpm) {
        const multipliers = [null, 240, 120, 60, 40, 30, 20, 15];
        const noteSymbols = ['', '𝅝', '𝅗𝅥', '𝅘𝅥', '𝅘𝅥₃', '𝅘𝅥𝅮', '𝅘𝅥𝅮₃', '𝅘𝅥𝅯'];
        const headers = [''];
        for (let i = 1; i < multipliers.length; i++) {
            const seconds = multipliers[i] / bpm;
            // Format to show 2 decimal places
            const formatted = seconds.toFixed(2);
            headers.push(noteSymbols[i] + ' ' + formatted + 's');
        }
        return headers;
    }

    // Update all grid headers when BPM changes
    updateAllGridHeaders() {
        const currentBPM = this.getCurrentBPM();
        const headers = this.calculateHeaders(currentBPM);
        
        // Update water grid
        this.updateGridHeaders('waterGrid', headers);
        // Update plucky grid
        this.updateGridHeaders('pluckyGrid', headers);
        // Update sub grid
        this.updateGridHeaders('subGrid', headers);
    }

    // Update headers for a specific grid
    updateGridHeaders(gridId, headers) {
        const grid = document.getElementById(gridId);
        if (!grid) return;
        ...
        allCells.forEach((cell, index) => {
            if (cell.classList.contains('col-header')) {
                // Column headers are in row 0, indices 1-7
                const col = index;
                if (col > 0 && col < headers.length) {
                    cell.textContent = headers[col];
                }
            } else if (cell.classList.contains('row-header')) {
                // Row headers are in column 0, indices 8, 16, 24, 32, 40, 48, 56
                const row = Math.floor(index / 8);
                if (row > 0 && row < headers.length) {
                    cell.textContent = headers[row];
                }
            }
        });
