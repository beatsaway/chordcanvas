(() => {
    ...
    function play({ events, durationSeconds, bpm = 0 }, onProgress, onDone, options = {}) {
      ...
      const isSamplePreset = preset0.type === "sample" && preset0.zones && typeof window !== "undefined" && window.InstrumentSampleHandler;
      const loadPromise = isSamplePreset
        ? window.InstrumentSampleHandler.ensurePresetLoaded(
            audioCtx,
            currentPreset,
            (typeof document !== "undefined" && document.baseURI) ? document.baseURI : (typeof window !== "undefined" && window.location && window.location.href) ? window.location.href : ""
          )
        : Promise.resolve();
          ...
          if (event.type === "noteOn") {
            ...
            if (preset.type === "sample" && preset.zones && window.InstrumentSampleHandler) {
              ...
              if (zone && zone.buffer) {
                ...
                const loopEnd = zone.loopEnd != null ? zone.loopEnd : Math.max(0.11, zone.buffer.duration - 0.1);
