/**
 * Cowbell synth B: square + saw + short "stick" noise transient for a more percussive hit.
 * Params: f1, f2, decay1, level, addSecondPair, secondF1, secondF2, secondLevel, secondDecay.
 */
(function () {
  ...
  function playCowbellB(synth, velocity, atTime, options, soundParams) {
    ...
    // Stick transient: short bandpassed noise at attack
    const stickNoise = synth._noise(0.022, true, seed + 10000);
    ...
    stickBp.frequency.value = 3200;
    stickBp.Q.value = 1.5;
    const stickG = synth.ctx.createGain();
    stickG.gain.setValueAtTime(v * 0.35, t);
    stickG.gain.exponentialRampToValueAtTime(0.001, t + 0.018);
    stickSrc.connect(stickBp).connect(stickG).connect(synth.master);
    stickSrc.start(t);
    stickSrc.stop(t + 0.022);
    ...
    if (humanize.detune && (humanize.detuneAmount ?? 0) > 0) {
      ...
      g1b.gain.setValueAtTime(v * 0.3, t);
      g1b.gain.exponentialRampToValueAtTime(0.001, t + decay1);
      g2b.gain.setValueAtTime(v * 0.15, t);
  ...
  window.playCowbellSynthB = playCowbellB;
})();
