...
const loopOptionsModal = document.getElementById('loopOptionsModal');
const playModeOptionsModal = document.getElementById('playModeOptionsModal');
const loopAllBtn = document.getElementById('loopAllBtn');
const loopThisBtn = document.getElementById('loopThisBtn');
...
let loopMode = 'all';
if (loopModeText?.textContent) {
    loopMode = loopModeText.textContent.trim() || 'all';
}
if (playModeText?.textContent) {
    isSynthMode = playModeText.textContent.trim() === 'synth';
}
...
function buildPanelGroupState(groupEl) {
    const state = {
        ...
        currentChordIndex: 0,
        synthBassPreset: 'warmPad',
        synthTreblePreset: 'warmPad',
        synthBassVolumeModPattern: DEFAULT_SYNTH_VOLUME_MOD,
        synthTrebleVolumeModPattern: DEFAULT_SYNTH_VOLUME_MOD,
...
class ChordPreviewItem {
    constructor({ chord, index, group }) {
        ...
        this.element.textContent = engine.getChordDisplayName(chord.rootNote, chord.chordType);
    ...
    updateFromGroup(group) {
        ...
        this.chordNotes = engine.chordToMIDINotes(this.chord);
...
function getSynthVolumeModPattern(group, part = 'bass') {
    if (part === 'treble') {
        return group?.synthTrebleVolumeModPattern || DEFAULT_SYNTH_VOLUME_MOD;
    }
    return group?.synthBassVolumeModPattern || DEFAULT_SYNTH_VOLUME_MOD;
}
...
function applySynthSettingsFromGroup(group, part = 'bass') {
    ...
    const preset = part === 'treble'
        ? (group?.synthTreblePresetSelect?.value || group?.synthTreblePreset || 'warmPad')
        : (group?.synthBassPresetSelect?.value || group?.synthBassPreset || 'warmPad');
...
async function startPreview({ fromLoop = false, startAt = null } = {}) {
    if (!fromLoop) {
        stopPreview();
    } else {
        clearPreviewTimers();
...
function syncGroupFromInputs(group, { stopOnEmpty = true } = {}) {
    ...
    if (group.synthBassPresetSelect) {
        group.synthBassPreset = group.synthBassPresetSelect.value || 'warmPad';
    }
    if (group.synthTreblePresetSelect) {
        group.synthTreblePreset = group.synthTreblePresetSelect.value || 'warmPad';
    }
    if (group.synthBassVolumeModSelect) {
        group.synthBassVolumeModPattern = group.synthBassVolumeModSelect.value || DEFAULT_SYNTH_VOLUME_MOD;
    }
    if (group.synthTrebleVolumeModSelect) {
        group.synthTrebleVolumeModPattern = group.synthTrebleVolumeModSelect.value || DEFAULT_SYNTH_VOLUME_MOD;
    }
...
function registerPanelGroup(groupEl) {
    ...
    if (group.bassVolumeSlider) {
        group.bassVolumeSlider.addEventListener('input', () => {
            updatePlaybackVolumeFromSliders(group);
            requestPreviewResync();
        });
    }
    if (group.highVolumeSlider) {
        group.highVolumeSlider.addEventListener('input', () => {
            updatePlaybackVolumeFromSliders(group);
            requestPreviewResync();
        });
    }
...
const helpSteps = [
    ...
    {
        text: 'Tap + to open chord presets or create a new panel.',
    ...
    {
        text: "Tap ALL or MIDI to change loop or play mode.",
        selector: '#previewToggle'
    },
    {
...
previewToggle.addEventListener('click', (event) => {
    if (event.target.closest('.play-mode-text')) return;
    if (isSequencePreviewing) {
        stopPreview();
    } else {
        startPreview().catch(() => {});
...
if (loopModeText && loopOptionsModal) {
    loopModeText.addEventListener('click', (event) => {
        event.stopPropagation();
        openModal(loopOptionsModal);
    });
    loopOptionsModal.addEventListener('click', (event) => {
        if (event.target === loopOptionsModal) {
            closeModal(loopOptionsModal);
        }
    });
}
...
if (loopAllBtn) {
    loopAllBtn.addEventListener('click', () => {
        loopMode = 'all';
        if (loopModeText) loopModeText.textContent = 'all';
        closeModal(loopOptionsModal);
    });
}

if (loopThisBtn) {
    loopThisBtn.addEventListener('click', () => {
        loopMode = 'this';
        if (loopModeText) loopModeText.textContent = 'this';
        closeModal(loopOptionsModal);
    });
}
...
if (playModeText && playModeOptionsModal) {
    playModeText.addEventListener('click', (event) => {
        event.stopPropagation();
        openModal(playModeOptionsModal);
    });
    playModeOptionsModal.addEventListener('click', (event) => {
        if (event.target === playModeOptionsModal) {
            closeModal(playModeOptionsModal);
        }
    });
}
...
if (playMidiBtn) {
    playMidiBtn.addEventListener('click', () => {
        isSynthMode = false;
        if (playModeText) playModeText.textContent = 'midi';
        closeModal(playModeOptionsModal);
    });
}
...
if (playSynthBtn) {
    playSynthBtn.addEventListener('click', () => {
        isSynthMode = true;
        if (playModeText) playModeText.textContent = 'synth';
        closeModal(playModeOptionsModal);
        if (isPreviewing || isSequencePreviewing) {
            stopPreview();
        }
