...
class PolySynthWrapper {
    constructor(options = {}) {
        ...
        // On mobile, AudioContext starts suspended - resume it if needed
        // This should be called from a user gesture handler
        if (this.audioCtx.state === 'suspended') {
            this.audioCtx.resume().catch(err => {
                console.warn('Failed to resume AudioContext in PolySynthWrapper constructor:', err);
            });
        }
    ...
    /**
     * Resume AudioContext (required on mobile devices)
     * This must be called from a user gesture event handler
     */
    resume() {
        if (this.audioCtx && this.audioCtx.state === 'suspended') {
            return this.audioCtx.resume().catch(err => {
                console.warn('Failed to resume PolySynthWrapper AudioContext:', err);
                return false;
            });
        }
        return Promise.resolve(this.audioCtx && this.audioCtx.state === 'running');
    }
}
