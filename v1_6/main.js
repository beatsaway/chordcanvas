...
scene.background = null; // Transparent background to show body bg color
...
camera.position.set(-0.065, 2.893, 2.748);
camera.rotation.set(-0.511, 0.008, 0.004);
camera.fov = 50;
...
controls.target.set(-0.097, 0.919, -0.775);
controls.update();
...
let masterVolumeGain = null; // Master volume control at final stage of audio chain
...
window.reconnectAudioChain = function() {
    ...
    // Connect tremolo effect if enabled (after all delays, before master volume)
    ...
    // Create master volume gain node if it doesn't exist (final stage of audio chain)
    if (!masterVolumeGain && synth && synth.synth && synth.synth.audioCtx) {
        masterVolumeGain = synth.synth.audioCtx.createGain();
        masterVolumeGain.gain.value = 0.5; // Default to 50% volume
    }
    
    // Connect through master volume gain (final stage before destination)
    if (masterVolumeGain && currentOutput) {
        if (currentOutput.connect) {
            currentOutput.connect(masterVolumeGain);
        }
        currentOutput = masterVolumeGain;
    }
    ...
    // Expose master volume control globally
    window.setMasterVolume = function(volume) {
        // Volume should be 0-1 (0 = silent, 1 = full volume)
        if (masterVolumeGain) {
            masterVolumeGain.gain.value = Math.max(0, Math.min(1, volume));
        }
    };
    
    window.getMasterVolume = function() {
        if (masterVolumeGain) {
            return masterVolumeGain.gain.value;
        }
        return 1.0;
    };
};
...
// Make pressKey and releaseKey available globally for chord input
if (typeof window !== 'undefined') {
...
// Make handleNoteOn available globally for chord input
if (typeof window !== 'undefined') {
...
// Make handleNoteOff available globally for chord input
if (typeof window !== 'undefined') {
...
// Make releaseAllNotes available globally for chord input
if (typeof window !== 'undefined') {
...
document.addEventListener('click', () => {
    ...
    // Note: MIDI and keypress input are disabled - using chord input instead
    // initializeMidiInput();
