...
if (randomByKeyModal) {
    randomByKeyModal.addEventListener('click', (event) => {
        if (event.target === randomByKeyModal) closeModal(randomByKeyModal);
    });
}
if (randomByKeyCancelBtn) {
    randomByKeyCancelBtn.addEventListener('click', () => closeModal(randomByKeyModal));
}
if (randomByKeyApplyBtn && randomByKeySelect && randomByKeyCount) {
    randomByKeyApplyBtn.addEventListener('click', () => {
        const keyName = randomByKeySelect.value || 'C';
        const count = parseInt(randomByKeyCount.value, 10) || 4;
        if (typeof window.ChordRan === 'undefined' || !window.ChordRan.generateChordString) return;
        const chordString = window.ChordRan.generateChordString(keyName, count);
        const group = getActiveGroup();
        if (group?.chordInput) {
