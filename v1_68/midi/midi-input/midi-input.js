...
(function() {
    ...
    window.midiInputSettings = {
        enabled: false // Default: OFF — permission only requested when user enables (has device)
    };
     ...
     * Does NOT request permission yet — only when user enables MIDI input (checkbox)
    ...
    window.initMidiInput = function(noteOnHandler, noteOffHandler, controlChangeHandler) {
        ...
        // Do NOT request permission here — only when user enables MIDI input
    };
    ...
    /**
     * Enable MIDI input — requests permission only when user enables (presumably has device)
     */
