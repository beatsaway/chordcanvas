...
(function() {
    ...
    const PanelIntegration = {
        ...
        setupSoundHandlers: function() {
            // Sound selection click handlers - add mobile touch support
            [1, 2, 3].forEach(soundNum => {
                ...
                if (output) {
                    ...
                    output.addEventListener('click', handleSoundClick);
                    output.addEventListener('touchstart', handleSoundClick, { passive: false });
                }
            });
        ...
        setupSoundbarHandlers: function() {
            // Setup click handlers for soundbar labels - add mobile touch support
            [1, 2, 3].forEach(soundNum => {
                ...
                if (label) {
                    ...
                    label.addEventListener('touchstart', handleLabelClick, { passive: false });
                }
                ...
                if (dice) {
                    ...
                    dice.addEventListener('touchstart', handleDiceClick, { passive: false });
                }
            });
        ...
        playCycle: async function(frequencies, bpm, cycleDuration, cycleRate, pitchShift, volume, regionType) {
            ...
            // Schedule next cycle only if still in same zone (no zone changes during drag)
            // This prevents CPU waste from rapid cycle creation when dragging between zones
            
            if (!this.shouldStop && this.isPlaying) {
                const nextBpm = parseInt(document.getElementById('bpmInput')?.value || 120);
                // Use current cycle parameters (not nextCycleRate) - cycles stay in same zone
                const nextCycleDuration = (this.currentCycleRate * 60) / (nextBpm * nextBpm);
                
                this.currentLoopTimeout = setTimeout(() => {
                    if (this.isPlaying && !this.shouldStop) {
                        // Continue with same parameters (same zone) - don't change zone during drag
                        // Get frequencies from current palette (real-time update)
                        const nextFrequencies = this.getChordFrequencies(this.currentRegionType);
                        this.playCycle(nextFrequencies, nextBpm, nextCycleDuration, this.currentCycleRate, this.currentPitchShift, this.currentVolume, this.currentRegionType);
                    } else {
                        this.isPlaying = false;
                        this.shouldStop = false;
        ...
        playSoundCycle: function(soundNum, generators, soundGain, frequencies, bpm, cycleDuration, cycleRate, pitchShift, volume, now, isLongCycle = false) {
            ...
            // Reduced delay from 0.001s (1ms) to 0.0001s (0.1ms) for more responsive playback
            const startTime = effectiveNow + 0.0001; // Minimal safety margin
            
            // Ensure startTime is not in the past (safety check)
            const safeStartTime = Math.max(startTime, actualCurrentTime + 0.0001);
            ...
            // Use volume parameter (from panel Y position) but scale up for better presence
            const scaledVolume = Math.min(1.0, volume * 1.2); // Scale up by 20% but cap at 1.0
            masterGain1.gain.setValueAtTime(scaledVolume, safeStartTime);
            ...
            masterGain2.gain.linearRampToValueAtTime(scaledVolume, safeStartTime + 0.1);
        ...
        handleTouchPoint: function(touchId, x, y) {
            ...
            if (this.activeTouches.size === 1) {
                ...
                if (!this.isPlaying) {
                    // Start playback in this zone
                    this.startPlayback(cycleRate, pitchShift, volume, regionIndex);
                } else {
                    // Already playing - check if zone changed
                    if (regionIndex !== this.currentRegionType) {
                        // Zone changed - stop playback (don't continue cycles in new zone)
                        this.stopPlayback();
                    }
                    // If same zone, cycles continue automatically with current parameters
                    // No need to update nextCycleRate/etc - cycles stay in same zone
                }
            } else {
                // Multi-touch: update active region based on first touch
                this.currentRegionIndex = firstTouch.regionIndex;
                this.updateActiveRegion(firstTouch.regionIndex);
                
                // If playing and zone changed, stop playback
                if (this.isPlaying && firstTouch.regionIndex !== this.currentRegionType) {
                    this.stopPlayback();
                }
            }
        },
