...
const chordItemTabType = document.getElementById('chordItemTabType');
const chordItemTabTreble = document.getElementById('chordItemTabTreble');
const chordItemPanelType = document.getElementById('chordItemPanelType');
const chordItemPanelTreble = document.getElementById('chordItemPanelTreble');
const chordItemTypeList = document.getElementById('chordItemTypeList');
const chordItemTypeConfirmBtn = document.getElementById('chordItemTypeConfirmBtn');
...
class ChordPreviewItem {
    constructor({ chord, index, group }) {
        ...
        this.element.addEventListener('contextmenu', (event) => {
            ...
            setCaretToChordIndex(this.group, this.index);
...
/** Segment index (0-based) of the comma-separated chunk containing the caret. */
function getCaretSegmentIndex(text, caretOffset) {
    if (!text || caretOffset < 0) return 0;
    return (text.substring(0, caretOffset).match(/,/g) || []).length;
}

/** Set the textarea caret to the start of the chord at the given index so it becomes the magenta (caret) chord. */
function setCaretToChordIndex(group, chordIndex) {
    if (!group?.chordInput || chordIndex < 0) return;
    const text = group.chordInput.value || '';
    const segments = text.split(',');
    let segmentIndex = -1;
    let count = 0;
    for (let i = 0; i < segments.length; i++) {
        if (engine.parseChord(segments[i].trim())) {
            if (count === chordIndex) {
                segmentIndex = i;
                break;
            }
    ...
    const startOffset = segmentIndex === 0 ? 0 : segments.slice(0, segmentIndex).join(', ').length + 2;
    group.chordInput.setSelectionRange(startOffset, startOffset);
    group.chordInput.focus();
    updateChordPreviewCaretFromInput(group);
}

/** Get chord preview index for the segment containing the caret. Returns -1 if that segment has no understood chord. */
function getCaretChordIndex(text, caretOffset) {
    if (!text || caretOffset < 0) return -1;
    const segmentIndex = (text.substring(0, caretOffset).match(/,/g) || []).length;
    const segments = text.split(',');
    if (segmentIndex >= segments.length) return -1;
    if (!engine.parseChord(segments[segmentIndex].trim())) return -1;
    let n = 0;
    for (let i = 0; i < segmentIndex && i < segments.length; i++) {
        if (engine.parseChord(segments[i].trim())) n++;
    }
    return n;
}

/** Triad quality for chord-type (for ideas panel: match diatonic C, Dm, … Bdim). */
function getTriadKey(chord) {
    if (!chord || !engine) return '';
    const t = chord.chordType || '';
    let triadSuffix = '';
    if (t.includes('diminished') || t === 'half-diminished-7th') triadSuffix = 'dim';
    else if (t === 'minor-triad' || (t.startsWith('minor-') && !t.includes('dim'))) triadSuffix = 'm';
    return (chord.rootNote || '') + triadSuffix;
}

function setChordPreviewCaretChord(group, index) {
    if (!group?.chordPreview) return;
    const items = group.chordPreview.querySelectorAll('.chord-preview-item');
    items.forEach((item) => item.classList.remove('caret-chord'));
    if (index >= 0) {
        const current = group.chordPreview.querySelector(`.chord-preview-item[data-index="${index}"]`);
        if (current) current.classList.add('caret-chord');
    }
    // Only update ideas panel when we have a valid chord (not on blur); show position in sequence
    if (index >= 0 && typeof window.updateIdeasCurrentChord === 'function') {
        const total = (group.chordSequence && group.chordSequence.length) || 0;
        const triadKey = group.chordSequence && group.chordSequence[index]
            ? getTriadKey(group.chordSequence[index])
            : '';
        window.updateIdeasCurrentChord(triadKey || null, index + 1, total);
    }
    // Show ideas panel the first time a chord turns magenta (caret in a chord) on this page load
    if (index >= 0 && typeof window.showIdeasPanelIfFirstCaretChord === 'function') {
        window.showIdeasPanelIfFirstCaretChord();
    }
}

function updateChordPreviewCaretFromInput(group) {
    if (!group?.chordInput || !group?.chordPreview) return;
    const text = group.chordInput.value || '';
    const caret = group.chordInput.selectionStart;
    const index = getCaretChordIndex(text, caret);
    setChordPreviewCaretChord(group, index);
}

function clearChordPreviewCaretChord(group) {
    setChordPreviewCaretChord(group, -1);
}

/** Replace the chord in the segment at caret with chordName; used by ideas panel Replace. */
function replaceCaretChordWith(chordName) {
    const group = getActiveGroup();
    if (!group?.chordInput || !chordName) return;
    const text = group.chordInput.value || '';
    const caret = group.chordInput.selectionStart;
    const segmentIndex = getCaretSegmentIndex(text, caret);
    const segments = text.split(',').map((s) => s.trim());
    if (segmentIndex >= segments.length) return;
    segments[segmentIndex] = chordName;
    const newValue = segments.join(', ');
    group.chordInput.value = newValue;
    const newCaret = segments.slice(0, segmentIndex).join(', ').length + chordName.length;
    group.chordInput.setSelectionRange(newCaret, newCaret);
    updateChordSequenceFromInput(group);
    updateChordPreviewCaretFromInput(group);
    if (typeof autoResizeTextarea === 'function') autoResizeTextarea(group.chordInput);
}

/** Insert chordName after the segment at caret; used by ideas panel Add. */
function addChordAfterCaret(chordName) {
    const group = getActiveGroup();
    if (!group?.chordInput || !chordName) return;
    const text = group.chordInput.value || '';
    const caret = group.chordInput.selectionStart;
    const segmentIndex = getCaretSegmentIndex(text, caret);
    const segments = text.split(',').map((s) => s.trim());
    const newSegments = segments.slice(0, segmentIndex + 1).concat([chordName], segments.slice(segmentIndex + 1));
    const newValue = newSegments.join(', ');
    group.chordInput.value = newValue;
    const newCaret = newSegments.slice(0, segmentIndex + 2).join(', ').length;
    group.chordInput.setSelectionRange(newCaret, newCaret);
    group.chordInput.focus();
    updateChordSequenceFromInput(group);
    updateChordPreviewCaretFromInput(group);
...
const VOLUME_MOD_OPTIONS = ['none', 'uphill', 'downhill', 'valley', 'hill', '2valley', '3valley', '4valley', '5valley', '6valley', '7valley', '8valley', '2hill', '3hill', '4hill', '5hill', '6hill', '7hill', '8hill'];
const PATTERN_OPTIONS = ['normal', 'ascend', 'descend', 'ascend2', 'descend2'];
const DELAY_MOD_OPTIONS = ['none', 'human', 'drunk'];
const SKIP_OPTIONS = [0, 2, 3, 4, 5, 6, 7, 8];
...
function randomizeActiveGroupRhythmAndSettings() {
    const group = getActiveGroup();
    if (!group) return;
    updateRhythmSelectOptions(group);
    if (group.bassRhythmSelect) {
        const bassVal = pickRandom(DURATION_MULTIPLIERS);
        group.bassRhythmSelect.value = String(bassVal);
        group.bassDurationMultiplier = bassVal;
    }
    if (group.highRhythmSelect) {
        const highVal = pickRandom(DURATION_MULTIPLIERS);
        group.highRhythmSelect.value = String(highVal);
        group.highDurationMultiplier = highVal;
    }
    if (group.patternSelect) {
        const pattern = pickRandom(PATTERN_OPTIONS);
        group.patternSelect.value = pattern;
        group.patternMode = pattern;
    }
    if (group.doubleHighNotes) {
        const double = pickRandom([1, 2]);
        group.doubleHighNotes.value = String(double);
        group.doubleHighEnabled = double === 2;
    }
    [group.skipSelect, group.skipSelect2, group.skipSelect3].filter(Boolean).forEach((sel) => {
        sel.value = String(pickRandom(SKIP_OPTIONS));
    });
    buildSkipPatternFromGroup(group);
    if (group.delayModSelect) {
        const delay = pickRandom(DELAY_MOD_OPTIONS);
        group.delayModSelect.value = delay;
        group.delayModPattern = delay;
    }
    if (group.bassVolumeModSelect) {
        const v = pickRandom(VOLUME_MOD_OPTIONS);
        group.bassVolumeModSelect.value = v;
        group.bassVolumeModPattern = v;
    }
    if (group.trebleVolumeModSelect) {
        const v = pickRandom(VOLUME_MOD_OPTIONS);
        group.trebleVolumeModSelect.value = v;
        group.trebleVolumeModPattern = v;
    }
    group.humanBassVolumeModIntensity = randomInt(20, 100) / 100;
    group.humanTrebleVolumeModIntensity = randomInt(20, 100) / 100;
    if (group.synthBassVolumeModSelect) {
        const v = pickRandom(VOLUME_MOD_OPTIONS);
        group.synthBassVolumeModSelect.value = v;
        group.synthBassVolumeModPattern = v;
    }
    if (group.synthTrebleVolumeModSelect) {
        const v = pickRandom(VOLUME_MOD_OPTIONS);
        group.synthTrebleVolumeModSelect.value = v;
        group.synthTrebleVolumeModPattern = v;
    }
...
async function playChordOnce(group, chord, index, skipPreviewHighlight) {
    ...
    if (!skipPreviewHighlight) setChordPreviewPlaying(group, index);
    isPreviewing = true;
    const delayState = { counter: 0 };
    ...
    setTimeout(() => {
        if (!isSequencePreviewing) {
            isPreviewing = false;
            if (!skipPreviewHighlight) setChordPreviewPlaying(group, null);
        }
...
/** Play a chord from the ideas panel (by chord name string). Uses active or first group. Does not highlight sequence preview. */
window.playIdeasChord = function (chordName) {
    const group = getActiveGroup();
    if (!group) return;
    const seq = engine.parseChordSequence(chordName);
    if (seq && seq[0]) playChordOnce(group, seq[0], 0, true);
};
...
function registerPanelGroup(groupEl, insertIndex) {
    ...
    if (group.chordInput) {
        ...
        group.chordInput.addEventListener('keyup', () => updateChordPreviewCaretFromInput(group));
        group.chordInput.addEventListener('click', () => updateChordPreviewCaretFromInput(group));
    }
...
function populateChordItemTypePanel() {
    if (!chordItemOptionsContext || !chordItemTypeList || !engine) return;
    const { chord } = chordItemOptionsContext;
    addChordState.mode = 'edit';
    addChordState.editGroup = chordItemOptionsContext.group;
    addChordState.editIndex = chordItemOptionsContext.index;
    ...
    chordItemTypeList.innerHTML = '';
    if (engine.chordTypeNames) {
        Object.entries(engine.chordTypeNames).forEach(([typeKey, suffix]) => {
            ...
            chordItemTypeList.appendChild(b);
        });
    }
}

function switchChordItemModalToTab(tabName) {
    const isType = tabName === 'type';
    chordItemTabType?.classList.toggle('active', isType);
    chordItemTabTreble?.classList.toggle('active', !isType);
    chordItemTabType?.setAttribute('aria-selected', isType ? 'true' : 'false');
    chordItemTabTreble?.setAttribute('aria-selected', !isType ? 'true' : 'false');
    chordItemPanelType?.classList.toggle('is-active', isType);
    chordItemPanelTreble?.classList.toggle('is-active', !isType);
    chordItemPanelType?.toggleAttribute('hidden', !isType);
    chordItemPanelTreble?.toggleAttribute('hidden', isType);
    if (!isType && chordItemOptionsContext) {
        ...
        if (trebleNotesModalTitle) trebleNotesModalTitle.textContent = 'Customise treble';
        if (trebleNotesInput) trebleNotesInput.value = '';
...
function openChordItemOptionsModal({ group, chord, index }) {
    ...
    populateChordItemTypePanel();
    switchChordItemModalToTab('type');
...
function openTrebleModal({ group, chord, index }) {
    ...
    chordItemOptionsContext = { group, chord, index };
    populateChordItemTypePanel();
    switchChordItemModalToTab('treble');
    openModal(chordItemOptionsModal);
}

function closeTrebleModal() {
    trebleModalState = null;
    if (chordItemOptionsContext) closeChordItemOptionsModal();
    else if (trebleNotesModal) closeModal(trebleNotesModal);
}
...
function playAddChordFeedbackNote(noteName) {
    ...
    playChordOnce(group, chord, 0, true);
}
...
if (addChordTypeList) {
    addChordTypeList.addEventListener('click', (event) => {
        ...
        if (addChordSelectedName) addChordSelectedName.textContent = 'Chord: ' + addChordState.chord.original;
        const group = getActiveGroup();
        if (group && addChordState.chord) playChordOnce(group, addChordState.chord, 0, true);
    });
}
if (addChordPlayBtn) {
...
if (chordItemTabType) {
    chordItemTabType.addEventListener('click', () => switchChordItemModalToTab('type'));
}
if (chordItemTabTreble) {
    chordItemTabTreble.addEventListener('click', () => switchChordItemModalToTab('treble'));
}
if (chordItemTypeList) {
    chordItemTypeList.addEventListener('click', (event) => {
        const btn = event.target.closest('.add-chord-type-btn');
        if (!btn || !btn.dataset.chordType) return;
        addChordState.chordType = btn.dataset.chordType;
        addChordState.chord = {
        ...
        chordItemTypeList.querySelectorAll('.add-chord-type-btn').forEach((b) => b.classList.remove('selected'));
        btn.classList.add('selected');
        const group = chordItemOptionsContext?.group || getActiveGroup();
        if (group && addChordState.chord) playChordOnce(group, addChordState.chord, 0, true);
    });
}
if (chordItemTypeConfirmBtn) {
    chordItemTypeConfirmBtn.addEventListener('click', () => {
        if (addChordState.mode !== 'edit' || !chordItemOptionsContext) return;
        const group = addChordState.editGroup;
        const index = addChordState.editIndex;
        const newChord = addChordState.chord;
        if (!group?.chordSequence || index < 0 || index >= group.chordSequence.length || !newChord) {
    ...
    if (randomByKeyPreview && group?.chordSequence?.length) {
        ...
        group.chordSequence.forEach((chord, index) => {
            ...
            item.addEventListener('click', (event) => {
                event.stopPropagation();
                playChordOnce(group, chord, index, true);
            });
...
const rhythmIdeasDiceBtn = document.getElementById('rhythmIdeasDiceBtn');
if (rhythmIdeasDiceBtn) {
    rhythmIdeasDiceBtn.addEventListener('click', () => {
        randomizeActiveGroupRhythmAndSettings();
    });
}

const SYNTH_PRESET_OPTIONS = ['warmPad', 'brightLead', 'softPiano', 'piano', 'guitar', 'epiano', 'bell', 'string', 'bass', 'pluck', 'organ'];

function randomizeActiveGroupSynthPresets() {
    const group = getActiveGroup();
    if (!group) return;
    if (group.synthBassPresetSelect) {
        const preset = pickRandom(SYNTH_PRESET_OPTIONS);
        group.synthBassPresetSelect.value = preset;
        group.synthBassPreset = preset;
    }
    if (group.synthTreblePresetSelect) {
        const preset = pickRandom(SYNTH_PRESET_OPTIONS);
        group.synthTreblePresetSelect.value = preset;
        group.synthTreblePreset = preset;
    }
    requestPreviewResync(group);
}
