...
(function() {
            ...
            <div id="settings-panel-sound" class="settings-panel" role="tabpanel">
                ...
                <div class="keyboard-visual-setting-item">
                    <label style="display:flex;align-items:center;gap:6px;">
                        <input type="checkbox" class="js-nostalgia-checkbox" title="Live playback tape/ramp effect into reverb (like WAV export)">
                        <span class="setting-label-inline">Nostalgia</span>
                    </label>
                </div>
                <div class="keyboard-visual-setting-item">
                    <label>
    ...
    function setupEventListeners() {
        ...
        document.body.addEventListener('change', function (e) {
            if (!e.target || !e.target.classList || !e.target.classList.contains('js-nostalgia-checkbox')) return;
            if (window.synth && window.synth.setNostalgiaMode) {
                var checked = e.target.checked;
                window.synth.setNostalgiaMode(checked);
                document.querySelectorAll('.js-nostalgia-checkbox').forEach(function (other) {
                    if (other !== e.target) other.checked = checked;
                });
            }
        });
        if (window.synth && window.synth.getNostalgiaMode) {
            var syncNostalgia = function () {
                var on = window.synth.getNostalgiaMode();
                document.querySelectorAll('.js-nostalgia-checkbox').forEach(function (cb) { cb.checked = !!on; });
            };
            if (document.getElementById('settings-nostalgia-checkbox')) syncNostalgia();
        }
    ...
    window.showKeyboardVisualSettings = function() {
        ...
        if (settingsModal) {
            if (settingsModal.id === 'app-popup') {
                ...
                if (window.synth && window.synth.getNostalgiaMode) {
                    var on = window.synth.getNostalgiaMode();
                    settingsModal.querySelectorAll('.js-nostalgia-checkbox').forEach(function (cb) { cb.checked = !!on; });
                }
            }
            updateUI();
        }
