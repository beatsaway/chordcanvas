...
class DrumMachine {
  ...
  _setupReverb() {
    // Optimized reverb: shorter length and mono for better performance
    const lengthSeconds = 0.8; // Reduced from 2.2s for better performance
    const decay = 0.7;
    const length = Math.min(this.ctx.sampleRate * lengthSeconds, 44100 * 0.8); // Cap at 44.1kHz equivalent
    const impulse = this.ctx.createBuffer(1, length, this.ctx.sampleRate); // Mono for performance
    ...
    this.reverbGain.gain.value = 0.25; // Slightly reduced reverb level
    this.reverbConvolver.connect(this.reverbGain);
  }

  _setupMultiMic() {
    // Simplified mic setup: close mic + room mic (overhead removed for performance)
    // Close mic (dry, direct) with EQ
    this.closeMic = this.ctx.createGain();
    this.closeMic.gain.value = 1.0; // Boosted to compensate for removed overhead mic
    this.overheadEQ = this.ctx.createBiquadFilter();
    this.overheadEQ.type = 'highshelf';
    this.overheadEQ.frequency.value = 5000;
    this.overheadEQ.gain.value = 2.5; // Moderate EQ boost
    this.closeMic.connect(this.overheadEQ);
    
    // Room mic (reverb only) - reduced for performance
    this.roomMic = this.ctx.createGain();
    this.roomMic.gain.value = 0.1; // Reduced from 0.15
  ...
  _routeToMics(node, pan = 0, delayOffset = 0, weights = {}) {
    ...
    // Simplified routing: close mic + room mic (overhead removed for performance)
    // Close mic (direct, no delay)
    const closeGain = this.ctx.createGain();
    closeGain.gain.value = close * 1.15; // Slight boost to compensate for removed overhead mic
    node.connect(closeGain);
    closeGain.connect(this.closeMic);
    
    // Room mic (reduced delay, simplified routing)
    const roomDelay = this.ctx.createDelay(0.01);
    roomDelay.delayTime.value = 0.003 + delayOffset; // Reduced delay
    const roomGain = this.ctx.createGain();
    roomGain.gain.value = room * 0.6; // Reduced room mic level
    node.connect(roomDelay);
    roomDelay.connect(roomGain);
    roomGain.connect(this.roomMic);
  }
