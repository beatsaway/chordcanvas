/**
 * DrumMachine - Advanced realistic drum synthesis with multi-mic, reverb, and natural resonances
...
class DrumMachine {
  constructor(options = {}) {
    ...
    // Options
    this.masterVolume = options.masterVolume || 0.3; // Reduced for mobile headroom (was 0.5)
  ...
  _setupReverb() {
    // Create room reverb impulse
    const lengthSeconds = 2.2;
    const decay = 0.6;
    const length = this.ctx.sampleRate * lengthSeconds;
    ...
    for (let channel = 0; channel < 2; channel++) {
      ...
      for (let i = 0; i < length; i++) {
        const n = length - i;
        channelData[i] = (Math.random() * 2 - 1) * Math.pow(n / length, decay) * (1 - i / length);
      }
    }
  ...
  _setupMultiMic() {
    // Close mic (dry, direct)
    this.closeMic = this.ctx.createGain();
    this.closeMic.gain.value = 0.85;
    
    // Overhead mic (bright, spatial)
    this.overheadMic = this.ctx.createGain();
    this.overheadMic.gain.value = 0.35;
    this.overheadEQ = this.ctx.createBiquadFilter();
    this.overheadEQ.type = 'highshelf';
    this.overheadEQ.frequency.value = 5000;
    this.overheadEQ.gain.value = 4;
    this.overheadMic.connect(this.overheadEQ);
    
    // Room mic (reverb only)
    this.roomMic = this.ctx.createGain();
    this.roomMic.gain.value = 0.15;
    this.roomMic.connect(this.reverbConvolver);
  }
  ...
  _setupMasterChain() {
    ...
    // Connect mics to master
    this.closeMic.connect(this.master);
    this.overheadEQ.connect(this.master);
  ...
  _routeToMics(node, pan = 0, delayOffset = 0, weights = {}) {
    const { close = 1, overhead = 1, room = 1 } = weights;
    
    // Close mic (direct, no delay)
    const closeGain = this.ctx.createGain();
    closeGain.gain.value = close;
    node.connect(closeGain);
    closeGain.connect(this.closeMic);
    
    // Overhead mic (slight delay ~1-2ms, slight pan variation)
    const overheadDelay = this.ctx.createDelay(0.01);
    overheadDelay.delayTime.value = 0.001 + delayOffset;
    const overheadGain = this.ctx.createGain();
    overheadGain.gain.value = overhead;
    const overheadPan = this.ctx.createStereoPanner();
    overheadPan.pan.value = pan * 0.6;
    node.connect(overheadDelay);
    overheadDelay.connect(overheadGain);
    overheadGain.connect(overheadPan);
    overheadPan.connect(this.overheadMic);
    
    // Room mic (longer delay ~5-10ms, more pan)
    const roomDelay = this.ctx.createDelay(0.02);
    roomDelay.delayTime.value = 0.005 + delayOffset * 2;
    const roomGain = this.ctx.createGain();
    roomGain.gain.value = room;
    const roomPan = this.ctx.createStereoPanner();
    roomPan.pan.value = pan * 1.2;
    node.connect(roomDelay);
    roomDelay.connect(roomGain);
    roomGain.connect(roomPan);
    roomPan.connect(this.roomMic);
  }
  ...
  playHat(open = false, velocity = 1, options = {}) {
    ...
    // Clearer reverb
    const reverbEQ = this.ctx.createBiquadFilter();
    reverbEQ.type = 'peaking';
    reverbEQ.frequency.value = 400;
    reverbEQ.Q.value = 1.5;
    reverbEQ.gain.value = -4.0;
    const reverbHighBoost = this.ctx.createBiquadFilter();
    reverbHighBoost.type = 'highshelf';
    reverbHighBoost.frequency.value = 8000;
    reverbHighBoost.gain.value = 3.0;
    
    const reverbSend = this.ctx.createGain();
    reverbSend.gain.value = 1.0;
    gain.connect(reverbEQ).connect(reverbHighBoost).connect(reverbSend);
    
    this._routeToMics(gain, pan, 0.0003, { close: 1.0, overhead: 1.2, room: open ? 0.8 : 0.6 });
    this._routeToMics(reverbSend, pan, 0.0003, { close: 0, overhead: 0, room: open ? 1.2 : 0.9 });
    
    const reverbToneSend = this.ctx.createGain();
    reverbToneSend.gain.value = open ? 0.9 : 0.7;
    const toneReverbEQ = this.ctx.createBiquadFilter();
    toneReverbEQ.type = 'peaking';
    toneReverbEQ.frequency.value = 500;
    toneReverbEQ.Q.value = 1.2;
    toneReverbEQ.gain.value = -3.5;
    const toneReverbHigh = this.ctx.createBiquadFilter();
    toneReverbHigh.type = 'highshelf';
    toneReverbHigh.frequency.value = 7000;
