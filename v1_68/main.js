...
if (typeof window !== 'undefined') {
    ...
    window.cameraSettings = window.cameraSettings || { spacebarCyclesViews: false, rOrbitsCamera: false };
}
...
// Lock to view 12 on load so user doesn't get free orbit before first interaction
const VIEW_12 = { position: { x: 0.025, y: 3.591, z: 2.092 }, target: { x: 0.025, y: 0.426, z: -0.547 } };
camera.position.set(VIEW_12.position.x, VIEW_12.position.y, VIEW_12.position.z);
controls.target.set(VIEW_12.target.x, VIEW_12.target.y, VIEW_12.target.z);
controls.enableRotate = false;
controls.enablePan = false;
controls.update();

// --- Smooth wheel zoom (dancer-style) ---
// OrbitControls wheel zoom can feel "steppy"; we instead lerp camera distance toward a desired zoom distance.
...
function onSmoothZoomWheel(e) {
    // Prevent page scroll / trackpad gestures from stealing wheel input.
    e.preventDefault();
    const deltaSign = e.deltaY > 0 ? 1 : -1;
    const factor = deltaSign > 0 ? ZOOM_OUT_FACTOR : ZOOM_IN_FACTOR;
...
renderer.domElement.addEventListener('wheel', onSmoothZoomWheel, { passive: false });
...
for (let i = 0; i < numWhiteKeys - 1; i++) {
    ...
    if (shouldHaveBlackKey(currentNote, nextNote)) {
        ...
        blackKey.userData.baseX = xPosition;
...
// Piano drag: horizontal drag moves the keyboard; vertical drag pans camera Y (drag up → camera lower, drag down → camera higher)
...
let pianoDragStartCameraY = 0;
let pianoDragStartTargetY = 0;
...
// World-space limits for camera.position.y while vertically panning (target moves by the same delta)
const CAMERA_PAN_Y_MIN = 0.85;
const CAMERA_PAN_Y_MAX = 6.25;
...
function applyPianoOffset() {
    keyMap.forEach(function (keyData) {
        ...
        if (typeof baseY === 'number') {
            ...
            keyData.originalY = baseY; // keep press/release in sync
        }
    });
}

function applyCameraVerticalPanDy(dy) {
    const rawDelta = dy * PIXELS_TO_WORLD;
    const d = Math.max(
        CAMERA_PAN_Y_MIN - pianoDragStartCameraY,
        Math.min(CAMERA_PAN_Y_MAX - pianoDragStartCameraY, rawDelta)
    );
    camera.position.y = pianoDragStartCameraY + d;
    controls.target.y = pianoDragStartTargetY + d;
    desiredZoomDistance = camera.position.distanceTo(controls.target);
    controls.update();
}

const pianoMouse = new THREE.Vector2();
const pianoRaycaster = new THREE.Raycaster();

function onPianoPointerDown(e) {
    if (isDraggingPiano) return;
    pianoMouse.x = (e.clientX / window.innerWidth) * 2 - 1;
    pianoMouse.y = -(e.clientY / window.innerHeight) * 2 + 1;
    pianoRaycaster.setFromCamera(pianoMouse, camera);
    ...
    pianoDragStartCameraY = camera.position.y;
    pianoDragStartTargetY = controls.target.y;
}
...
function onPianoPointerUp() {
    isDraggingPiano = false;
}

renderer.domElement.addEventListener('pointerdown', onPianoPointerDown);
renderer.domElement.addEventListener('pointermove', onPianoPointerMove);
renderer.domElement.addEventListener('pointerup', onPianoPointerUp);
renderer.domElement.addEventListener('pointercancel', onPianoPointerUp);
...
function animate() {
    ...
    // Smooth zoom: if idle orbit is active, lerp its radius; otherwise lerp camera distance along view direction.
    if (window.cameraIdleOrbit && window.cameraIdleOrbit.active && window.camera && window.controls) {
        const o = window.cameraIdleOrbit;
        const currentRad = (o.radius != null) ? o.radius : camera.position.distanceTo(controls.target);
        const newRad = currentRad + (desiredZoomDistance - currentRad) * ZOOM_LERP;
        o.radius = Math.max(controls.minDistance, Math.min(controls.maxDistance, newRad));
        desiredZoomDistance = o.radius;
    } else {
        ...
        desiredZoomDistance = Math.max(controls.minDistance, Math.min(controls.maxDistance, newDist));
        if (currentDist > 0.000001) {
            zoomDir.normalize().multiplyScalar(desiredZoomDistance);
...
// Set master volume when GSL synth has created its context (default 700%, range 0–2000%)
if (synth && synth.setMasterVolume) {
    synth.setMasterVolume(700);
}
...
function releaseAllNotes() {
    // Release sustain pedal so sustained notes and app state are cleared
    sustainPedalActive = false;
    if (synth && synth.synth && typeof synth.synth.setSustainPedal === 'function') {
        synth.synth.setSustainPedal(false);
    }
...
if (typeof window !== 'undefined') {
    window.releaseAllNotes = releaseAllNotes;
}
...
// First click: match v1_12 — do NOT call reconnectAudioChain (synth connects itself in ensureContext on first note)
document.addEventListener('click', () => {
    synth = window.synth || synth;
    if (!synth) {
        initializeSynth();
        initializeMidiMapping();
    } else if (synth !== window.synth) {
        initializeMidiMapping();
    }
    ...
    function afterAudioReady() {
        ...
        if (window.applyCameraView) {
            if (typeof window.currentCameraViewIndex !== 'number') {
                var viewIndex = document.body.classList.contains('tapmode') ? 13 : 12;
                window.currentCameraViewIndex = viewIndex;
                window.applyCameraView(viewIndex);
            }
        }
        if (window.controls) {
...
// Spacebar: cycle to next camera view (when enabled in Keyboard > Camera)
...
// Keyboard shortcut to release all stuck notes (press 'R' key); R also toggles orbit when enabled in Keyboard > Camera
window.addEventListener('keydown', (event) => {
...
function populateGslPresetDropdown() {
    ...
    return handler.ensureGslManifest().then(function (list) {
        ...
        if (!list || !list.length) {
            const defaultSlug = 'gsl_piano';
            if (typeof window.ccApplyCurrentRowSoundToGlobals === 'function') {
                window.ccApplyCurrentRowSoundToGlobals();
            }
            window.currentGslPreset = (window.gslPresetSlots && window.gslPresetSlots[0]) || defaultSlug;
            if (sel) sel.value = window.currentGslPreset;
        ...
        if (!window.gslPresetSlots || !window.gslPresetSlots.length || !window.gslPresetSlots[0]) {
            window.gslPresetSlots = [defaultSlug, null, null, defaultSlug, null, null];
            if (!window.gslSlotVolumes || window.gslSlotVolumes.length < 6) {
                window.gslSlotVolumes = [66, 33, 33, 66, 33, 33];
            }
        }
        window.currentGslPreset = (window.gslPresetSlots && window.gslPresetSlots[0]) || defaultSlug;
        if (sel) sel.value = window.currentGslPreset;
...
function applySoundPreset(presetName) {
    ...
    if (isGsl) {
        ...
        window.gslSlotVolumes = [66, 33, 33, 66, 33, 33];
        startBackgroundPreloadSlots();
        if (typeof window.ccSaveGlobalsToCurrentRowSound === 'function') window.ccSaveGlobalsToCurrentRowSound();
 ...
 * Only elements that exist in index.html are synced; physics enable-* checkboxes are not in this build.
 */
