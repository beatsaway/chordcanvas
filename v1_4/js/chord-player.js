...
(function() {
    ...
    class ChordPlayer {
        ...
        playChord(rootNote, chordType, durationSec) {
            // Check if palette sound is enabled
            const playPaletteSound = window.app && window.app.ui ? window.app.ui.getPlayPaletteSound() : true;
            
            // If palette sound is disabled, don't play anything (just show visual dots)
            if (!playPaletteSound) {
                return;
            }

            // Get palette frequencies directly from intervals
            const frequenciesToPlay = PianoVisualizer.getChordPaletteFrequenciesFromChord(rootNote, chordType);
            if (frequenciesToPlay.length === 0) return;

            // Play the frequencies with much quieter volume (0.1 = 10% of normal volume)
            frequenciesToPlay.forEach(freq => {
                this.waterSynth.playNote(freq, durationSec, 0.02, 0.2, 0.03, 0.7, 0.1);
            });
        }
