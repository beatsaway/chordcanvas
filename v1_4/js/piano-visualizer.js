...
(function() {
    ...
    const PianoVisualizer = {
        ...
        init: function(containerId) {
            ...
            // Add resize listener to update piano scale
        ...
        createPiano: function() {
            ...
            setTimeout(() => {
                ...
                // Scale piano to fill 100vw if smaller
                this.scalePianoToFit();
            }, 0);
        },
        ...
        /**
         * Scale piano keys to fill 100vw when there's space, maintaining proportions
         */
        scalePianoToFit: function() {
            const pianoKeys = this.pianoContainer?.querySelector('.piano-keys');
            if (!pianoKeys) return;
            
            // Get natural width (without any transforms)
            const naturalWidth = pianoKeys.scrollWidth;
            const viewportWidth = window.innerWidth;
            
            // If natural width is less than viewport, scale up to fill
            if (naturalWidth > 0 && naturalWidth < viewportWidth) {
                const scale = viewportWidth / naturalWidth;
                pianoKeys.style.width = viewportWidth + 'px';
                pianoKeys.style.transform = `scaleX(${scale})`;
            } else {
                // Reset if natural width is already >= viewport
                pianoKeys.style.width = '';
                pianoKeys.style.transform = '';
            }
        },
