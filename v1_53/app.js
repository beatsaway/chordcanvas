...
function getPatternNotesForCycle(ordered, direction, cycleIndex) {
    ...
    if (direction === 'ascend2') {
        const start = (cycleIndex * 2) % length;
        const second = (start + 1) % length;
        return [ordered[start], ordered[second]];
    }
    if (direction === 'descend2') {
        const start = (cycleIndex * 2) % length;
        const firstIndex = length - 1 - start;
        const secondIndex = length - 1 - ((start + 1) % length);
...
function scheduleNextChord(forcedStartTime = null) {
    ...
    if (patternMode === 'normal') {
        schedulePreviewNotes(chordNotes.high, startTime, endTime, highCycleSeconds, barSeconds, 0.24, { applySkip: true });
    } else {
        schedulePreviewArpeggio(chordNotes.high, startTime, endTime, highCycleSeconds, barSeconds, 0.24, patternMode, { applySkip: true });
    }
...
function playChordOnce(chord, index) {
    ...
    const chordNotes = engine.chordToMIDINotes(chord);
    const notes = [...chordNotes.bass, ...chordNotes.high];
...
function updateCurrentHighNotes(index) {
    ...
    const highNames = chordNotes.high.map(midiToNoteName);
