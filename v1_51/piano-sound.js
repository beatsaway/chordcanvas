...
(function() {
    ...
    function playAllNotesAtOnce(notes, volumeDB = 0, duration = 0.5) {
        ...
        // Play immediately - Tone.js is already started by playChordNotesSequentially
        const now = Tone.now();
        const startTime = now + 0.001; // Minimal delay (1ms) to ensure audio nodes are ready
    ...
    // Start Tone.js proactively on first user interaction
    function startToneProactively() {
        if (window.Tone && Tone.context.state !== 'running') {
            Tone.start().catch(err => {
                console.warn('Failed to start Tone.js proactively:', err);
            });
        }
    }
    
    // Start Tone.js on first user interaction (click, touch, or keypress)
    const startToneOnce = () => {
        startToneProactively();
        // Remove listeners after first interaction
        document.removeEventListener('click', startToneOnce);
        document.removeEventListener('touchstart', startToneOnce);
        document.removeEventListener('keydown', startToneOnce);
    };
    
    document.addEventListener('click', startToneOnce, { once: true });
    document.addEventListener('touchstart', startToneOnce, { once: true });
    document.addEventListener('keydown', startToneOnce, { once: true });
