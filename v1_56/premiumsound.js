(() => {
    ...
    function setPreset(name) {
      currentPreset = PRESETS[name] ? name : "warmPad";
    }
    ...
    function play({ events, durationSeconds }, onProgress, onDone, options = {}) {
      ...
      const preset = PRESETS[currentPreset] || PRESETS.warmPad;
      ...
      events.forEach((event) => {
        ...
        if (event.type === "noteOn") {
          const velocity = Math.max(0.02, event.velocity / 127);
          ...
          if (preset.filter) {
            ...
            const cutoff = preset.filter.base + preset.filter.velocity * velocity;
          ...
          const peak = velocity;
          const sustain = velocity * preset.sustain;
          ...
          if (preset.noise) {
            ...
            noiseGain.gain.setValueAtTime(velocity * preset.noise, eventTime);
          ...
          list.push({ gain, oscillators, sustain });
    ...
    function scheduleEvents(ctx, nodes, noise, preset, events, startOffset = 0) {
      if (!events || events.length === 0) return;
    ...
    function renderBuffer(segments, { sampleRate = 44100 } = {}) {
      ...
      segments.forEach((segment) => {
        const preset = PRESETS[segment.preset] || PRESETS.warmPad;
