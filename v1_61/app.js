...
const chordTypesClose = document.getElementById('chordTypesClose');
...
function openChordTypes() {
    ...
    Object.entries(names).forEach(([typeKey, suffix]) => {
        ...
        chordTypesList.appendChild(nameEl);
        chordTypesList.appendChild(exEl);
    });
...
if (chordTypesClose && chordTypesModal) {
    chordTypesClose.addEventListener('click', closeChordTypes);
...
function applyRandomByKeyGenerate() {
    ...
    const styleId = randomByKeyStyleSelect ? randomByKeyStyleSelect.value : '';
    const chordString = window.ChordRan.generateChordString(keyName, count, styleId);
    const group = getActiveGroup();
    if (group?.chordInput) {
